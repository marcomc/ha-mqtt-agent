...
- `publish-once --dry-run` exact message rendering without MQTT publishes or
  state writes.
- Explicit device-scoped MQTT discovery cleanup command.
...
`cleanup-discovery --current` removes retained discovery topics generated by
the current capability model for the configured `device_id`. It is intended for
retiring an integration and does not enumerate broker topics or affect other
devices.
...
- Device-scoped current and legacy `0.1.x` cleanup topics.
