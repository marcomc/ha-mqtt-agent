...
- [Raspberry Pi Under-Voltage Alerts](#raspberry-pi-under-voltage-alerts)
- [Step 5: Add macOS Energy to the Energy Dashboard](#step-5-add-macos-energy-to-the-energy-dashboard)
- [Step 6: Authorize macOS Wi-Fi SSID Access](#step-6-authorize-macos-wi-fi-ssid-access)
...
Home Assistant MQTT Agent publishes the local host as a Home Assistant MQTT
device. Home Assistant reads retained MQTT discovery messages, creates the
device, and then uses the retained state topic for supported local sensors.

For the service-to-MQTT runtime flow, see
[Runtime Flow](../README.md#runtime-flow).

```mermaid
flowchart LR
  accTitle: Home Assistant setup flow
  accDescr: Shows the setup path from MQTT integration to discovered entities.
  ...
  device --> dashboard["Add supported entities to dashboards"]
  dashboard --> agent["Run platform service continuously"]
```

## Prerequisites

- A macOS, Linux, or Raspberry Pi OS host.
- Python `3.11` or newer.
- Xcode Command Line Tools with `make`, `swiftc`, and `codesign` for macOS
  source installs.
- `systemd` and setup-time `sudo` for Linux service installs.
...
- This host can reach the broker hostname and port configured in the installed
  config.
...
On macOS, install Xcode Command Line Tools first if `make`, `swiftc`, or
`codesign` is missing:
...
while the broker remains unreachable and lets the service resume promptly
...
background service while you are checking MQTT discovery.
...
If the service is already installed, restart it after changing this file:
...
4. Confirm that the device has these entities:
   ...
   - Raspberry Pi raw throttle flags, current conditions, and firmware history
     when `vcgencmd` and compatible firmware are available
   - Raspberry Pi 5-family input voltage from the PMIC `EXT5V_V` ADC
...
## Raspberry Pi Under-Voltage Alerts

Use Home Assistant as the alert authority. The current under-voltage binary
sensor provides the immediate, actionable condition.

Replace the example entity and notification service IDs with those created in
your Home Assistant instance:

```yaml
alias: Raspberry Pi under-voltage detected
triggers:
  - trigger: state
    entity_id: binary_sensor.rpi_rpi_under_voltage
    to: "on"
actions:
  - action: notify.mobile_app_iphone
    data:
      title: Raspberry Pi power warning
      message: Current under-voltage reported by {{ trigger.to_state.name }}.
mode: queued
```

For repeating notifications while under-voltage remains active, configure the
Home Assistant Alert integration against
`binary_sensor.rpi_rpi_under_voltage`.

Apple Home is not recommended for this alert. Home Assistant's HomeKit Bridge
does not natively map the `problem` device class and excludes diagnostic
entities by default; use Home Assistant Companion notifications on Apple
devices instead.

The firmware provides no event count or timestamp. The Linux
`raspberrypi-hwmon` driver can clear sticky history bits between agent samples,
so the agent intentionally does not synthesize a count, latest timestamp, or
new-event entity from periodic polling. Those features require a continuous
kernel-event collector. Raspberry Pi Zero and original Model A/B boards omit
under-voltage entities because they lack the detector. See
[Raspberry Pi Throttle Status Research](raspberry-pi-throttle-status.md).

On Raspberry Pi 5-family hardware, use `sensor.rpi_rpi_input_voltage` for
voltage history and dashboard graphs. Keep the current under-voltage binary
sensor as the primary alert source because it applies the Raspberry Pi
firmware's supported detection threshold and tolerance.

## Step 5: Add macOS Energy to the Energy Dashboard

The Energy dashboard needs an energy entity, not only a power entity. On macOS,
use the `Energy` entity published by this app, because it is reported in `kWh`
with `state_class: total_increasing`. Linux does not publish an estimated
energy counter.

1. Go to **Settings > Dashboards > Energy**.
2. Open the individual devices section.
3. Add a device energy consumption entry.
4. Select the macOS energy entity, for example `sensor.workstation_energy`.
...
## Step 6: Authorize macOS Wi-Fi SSID Access
...
Install or restart the platform service:
...
The service reads config only at process startup, so config changes are not
picked up until the service is restarted.

## Troubleshooting

### The device does not appear

- Run `ha-mqtt-agent doctor`.
- Run `ha-mqtt-agent publish-once --dry-run` and confirm the expected discovery
  topics are rendered.
...
### The service is running but publishing fails
...
publish failure, the service uses lightweight broker connection probes before
doing another full telemetry publish, and the retry backoff is capped at
60 seconds.
...
- Restart the service after changing either value.
...
Linux CPU temperature is published when a readable thermal zone exists. The
default macOS LaunchAgent intentionally runs without root privileges. Deeper
macOS thermal channels such as CPU, GPU, memory, SSD, palm-rest, and fan RPM
usually require a privileged sensor source and are not published by the default
service.
...
- [Raspberry Pi `get_throttled` documentation](https://www.raspberrypi.com/documentation/computers/os.html#get-throttled)
