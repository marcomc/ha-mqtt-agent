...
4. Confirm that the device has these entities:
   ...
   - Raspberry Pi 5 input voltage from the PMIC `EXT5V_V` ADC
...
On Pi 5, use `sensor.rpi_rpi_input_voltage` for voltage history and dashboard
graphs. Keep the current under-voltage binary sensor as the primary alert
source because it applies the Raspberry Pi firmware's supported detection
